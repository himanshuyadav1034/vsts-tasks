			...
			promises.push(validateTask(folderName, task)
				.then(function () {
					...
					if (task.execution['Node']) {
						...
						var libSource = path.join(_downloadPath, 'npm', 'vsts-task-lib', libVer, '*');
						shell.cp('-R', libSource, tgtPath);
					}
					...
					if (task.execution['PowerShell3']) {
						...
						var libSource = path.join(_downloadPath, 'npm', 'vsts-task-sdk', libVer, 'node_modules', 'vsts-task-sdk', 'VstsTaskSdk', '*');
