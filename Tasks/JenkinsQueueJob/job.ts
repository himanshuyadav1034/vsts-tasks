        ...
        if (this.working) { // return if already working
            ...
            setTimeout(() => {
                if (this.state == JobState.New) {
                    ...
                    this.stopWork(this.queue.taskOptions.pollInterval, null);
                }
            }, this.workDelay);
        }
    }
    ...
    initialize() {
        ...
        thisJob.search.initialize().then(() => {
            if (thisJob.search.initialized) {
                ...
                var nextWorkDelay = (newState == JobState.Locating) ? thisJob.queue.taskOptions.pollInterval : thisJob.workDelay;
                thisJob.stopWork(nextWorkDelay, newState);
            } else {
                //search not initialized, so try again
                thisJob.stopWork(thisJob.queue.taskOptions.pollInterval, thisJob.state);
            }
        }).fail((err) => {
            throw err;
        });
